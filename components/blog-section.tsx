...
export function BlogSection() {
  return (
    <SectionContainer id="blog" className="relative py-28">
        ...
        {blogPosts.map((post, idx) => (
          <motion.div
            ...
            className="rounded-2xl border border-slate-800/70 bg-slate-900/60 
                       backdrop-blur p-6 shadow-xl hover:border-sky-400/40 
                       hover:bg-slate-900/80 transition-all"
          >
            <span className="text-xs px-2 py-1 rounded-md bg-slate-800 text-sky-300">
              {post.tag}
            </span>
            ...
            <p className="mt-2 text-sm text-slate-300/90">{post.description}</p>
